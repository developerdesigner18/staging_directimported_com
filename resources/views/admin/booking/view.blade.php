            ...
            <div class="modal-content">
                ...
                <form id="documentVerifiedMailForm">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light border rounded px-4"
                                data-bs-dismiss="modal">
                            Cancel
                        </button>

                        <button type="submit" id="submitDocumentMailBtn"
                                class="btn btn-danger text-white rounded px-4">
...
@push('style')
    ...
    <style>
        ...
        .document-image-wrapper {
            position: relative;
            width: 100%;
            padding-top: 63%; /* ~1.58:1 Aspect Ratio (Bank card/ID format) */
            overflow: hidden;
            background: #e9ecef;
            border-radius: 4px;
        }
        ...
        .document-image {
            width: 100%;
            height: 100%;
            object-fit: contain; /* Ensures full document is visible without cropping */
        ...
        <div class="card-body">
            ...
            @foreach(\App\Enum\BookingStatus::cases() as $status)
                <option value="{{ $status->value }}"
                    {{ $rowData->status->value == $status->value ? 'selected' : '' }}>
            ...
            @endforeach
                        ...
                        <tr>
                            <th>First Name</th>
                            <td><input type="text" name="first_name" class="form-control" value="{{ $rowData->first_name??'' }}"></td>
                        </tr>
                        <tr>
                            <th>Last Name</th>
                            <td><input type="text" name="last_name" class="form-control" value="{{ $rowData->last_name??'' }}"></td>
                        </tr>
                        <tr>
                            <th>Email</th>
                            <td>{{ $rowData->email??'' }}</td>
                        ...
                        <tr>
                            <th>Start Date</th>
                            <td>
                                <input type="date" name="start_date" class="form-control" value="{{ date('Y-m-d',strtotime($rowData->start_date)) }}">
                            </td>
                        </tr>
                        <tr>
                            <th>End Date</th>
                            <td>
                                <input type="date" name="end_date" class="form-control" value="{{ date('Y-m-d',strtotime($rowData->end_date)) }}">
                            </td>
                        </tr>
                        <tr>
                            <th>Pickup Time</th>
                            <td>
                                <input type="time" name="start_time" class="form-control" value="{{ date('H:i',strtotime($rowData->start_time)) }}">
                            </td>
                        </tr>
                        <tr>
                            <th>Dropoff Time</th>
                            <td>
                                <input type="time" name="end_time" class="form-control" value="{{ date('H:i',strtotime($rowData->end_time)) }}">
                            </td>
                        </tr>
                        <tr>
                            <th>Location</th>
                            <td>
                                <input type="text" name="location" class="form-control" value="{{ $rowData->location }}" readonly>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                ...
                                <select class="form-select select2-multiple" name="included_accessories[]" multiple>
                                    ...
                                    @foreach($freeList as $acc)
                                        <option value="{{ $acc->id }}"
                                            {{ in_array($acc->id, $freeAccessoryIds) ? 'selected' : '' }}>
                                            {{ $acc->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td>
                                <select class="form-select select2-multiple" name="selected_accessories[]" multiple>
                                    @foreach($extraList as $acc)
                                        <option value="{{ $acc->id }}"
                                            {{ in_array($acc->id, $extraAccessoryIds) ? 'selected' : '' }}>
                                            {{ $acc->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea rows="3" class="form-control" name="system_comment">{{ $rowData->system_comment }}</textarea>
                            </td>
                        </tr>
                        <tr>
                            <th>Email Comments</th>
                            <td>
                                <textarea rows="3" class="form-control" name="email_comment">{{ $rowData->email_comment }}</textarea>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <div class="input-group">
                                    ...
                                    <input type="number" step="any" value="{{ isset($rowData->price) ? round($rowData->price) : '0' }}" class="form-control" name="price">
                                </div>
                            </td>
                        </tr>
                        <tr>
...
{{--                            <td colspan="2">--}}
{{--                                <button type="button" id="" class="btn btn-soft-danger waves-effect waves-light" >Send Payment Link to Customer</button>--}}
{{--                                <button type="button"--}}
{{--                                        class="btnSendBookingDetail btn btn-soft-danger"--}}
...
{{--                                        data-fname="{{ $rowData->first_name }}" data-lname="{{ $rowData->last_name }}"><i--}}
{{--                                        class="bx bx-loader spinner me-2" style="display: none" id="btnSendLoginDetailSpinner"></i>Send--}}
{{--                                    Login Details--}}
{{--                                </button>--}}

{{--                                --}}{{--                     <a href="{{ route('admin.booking.contract-preview',$rowData->booking_id) }}" class="btn btn-soft-danger waves-effect waves-light" id="contractPreview">Contract Preview</a>--}}
{{--                                <button type="button"--}}
{{--                                        class="documentVerifiedMail btn btn-soft-danger"--}}
{{--                                        data-booking-id="{{ $rowData->booking_id }}"--}}
{{--                                >--}}
{{--                                    <i class="bx bx-loader spinner me-2" style="display: none" id="btnSendDocumentVerifiedMailSpinner"></i>--}}
...
{{--                            </td>--}}
                            <td colspan="2">
                                ...
                                <div class="d-flex flex-wrap gap-2 align-items-center">
                                    ...
                                    <button type="button"
                                        ...
                                        <input type="checkbox"
                                               class="form-check-input ms-2 bulk-action"
                ...
                <button type="submit" id="submitBtn" class="btn btn-success"><i class="bx bx-loader spinner me-2" style="display: none" id="processingBtnSpinner"></i>Update</button>
            </form>
        </div>
    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5 mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="password" id="manual_password" class="form-control" placeholder="Enter new password">
                            </div>
                        </div>
                        <div class="col-md-2 mb-3">
                            <label class="form-label d-none d-md-block">&nbsp;</label>
                            <button type="submit" class="btn btn-primary w-100" id="btnUpdatePassword">
                                <i class="bx bx-loader spinner me-2" style="display: none" id="btnUpdatePasswordSpinner"></i>Update
                            </button>
                        </div>
                    </div>
                    <div class="row px-1">
                        <div class="col-12">
                            <small class="text-muted"><i class="bx bx-info-circle me-1"></i>Enter a new password above to manually override the customer's current password.</small>
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <div class="document-card-inner">
                            <div class="document-header">
                                ...
                                @if(!empty($rowData->user?->userDetail?->passport_status))
                                    <span class="status-badge status-{{ strtolower($rowData->user->userDetail->passport_status) }}">
                                        {{ $rowData->user->userDetail->passport_status }}
                                    </span>
                                @endif
                            </div>
                            <div class="document-image-wrapper">
                                {{--                                <a href="{{ $rowData->user?->userDetail?->passport ?: asset('uploads/user_documents/default.jpg') }}" target="_blank" data-lightbox="gallery" class="document-link">--}}
                                <a href="javascript:void(0);"
                                   class="document-link openPreview"
                                   ...
                                   data-user_id="{{ $rowData->user?->id }}"
                                   data-field="passport"
                                   data-has-image="{{ $rowData->user?->userDetail?->getRawOriginal('passport') ? 1 : 0 }}"
                                   data-status="{{ $rowData->user?->userDetail?->passport_status }}">
                                    <img class="document-image" src="{{ $rowData->user?->userDetail?->passport ?: asset('uploads/user_documents/default.jpg') }}" alt="Passport">
                        ...
                        <div class="document-card-inner">
                            <div class="document-header">
                                ...
                                @if(!empty($rowData->user?->userDetail?->international_lic_status))
                                    <span class="status-badge status-{{ strtolower($rowData->user->userDetail->international_lic_status) }}">
                                        {{ $rowData->user->userDetail->international_lic_status }}
                                    </span>
                                @endif
                            </div>
                            <div class="document-image-wrapper">
                                {{--                                <a href="{{ $rowData->user?->userDetail?->international_lic ?: asset('uploads/user_documents/default.jpg') }}" target="_blank" data-lightbox="gallery" class="document-link">--}}
                                <a href="javascript:void(0);"
                                   class="document-link openPreview"
                                   ...
                                   data-status="{{ $rowData->user?->userDetail?->international_lic_status }}">
                                    <img class="document-image" src="{{ $rowData->user?->userDetail?->international_lic ?: asset('uploads/user_documents/default.jpg') }}" alt="International License Front">
                                    <div class="document-overlay">
                                        <i class="bx bx-search-alt"></i>
                                        <span>View Full Size</span>
                                    </div>
                        ...
                        <div class="document-card-inner">
                            <div class="document-header">
                                ...
                                @if(!empty($rowData->user?->userDetail?->international_lic_back_status))
                                    <span class="status-badge status-{{ strtolower($rowData->user->userDetail->international_lic_back_status) }}">
                                        {{ $rowData->user->userDetail->international_lic_back_status }}
                                    </span>
                                @endif
                            </div>
                            <div class="document-image-wrapper">
                                {{--                                <a href="{{ $rowData->user?->userDetail?->international_lic_back ?: asset('uploads/user_documents/default.jpg') }}" target="_blank" data-lightbox="gallery" class="document-link">--}}
                                <a href="javascript:void(0);"
                                   class="document-link openPreview"
                                   ...
                                   data-id="{{ $rowData->user?->userDetail?->id }}"
                                   data-user_id="{{ $rowData->user?->id }}"
                                   data-field="international_lic_back"
                                   ...
                                   data-status="{{ $rowData->user?->userDetail?->international_lic_back_status }}">
                                    <img class="document-image" src="{{ $rowData->user?->userDetail?->international_lic_back ?: asset('uploads/user_documents/default.jpg') }}" alt="International License Back">
                                    <div class="document-overlay">
                                        <i class="bx bx-search-alt"></i>
                                        <span>View Full Size</span>
                                    </div>
                        ...
                        <div class="document-card-inner">
                            <div class="document-header">
                                ...
                                @if(!empty($rowData->user?->userDetail?->regular_lic_status))
                                    <span class="status-badge status-{{ strtolower($rowData->user->userDetail->regular_lic_status) }}">
                                        {{ $rowData->user->userDetail->regular_lic_status }}
                                    </span>
                                @endif
                            </div>
                            <div class="document-image-wrapper">
                                {{--                                <a href="{{ $rowData->user?->userDetail?->regular_lic ?: asset('uploads/user_documents/default.jpg') }}" target="_blank" data-lightbox="gallery" class="document-link">--}}
                                <a href="javascript:void(0);"
                                   class="document-link openPreview"
                                   ...
                                   data-status="{{ $rowData->user?->userDetail?->regular_lic_status }}">
                                    <img class="document-image" src="{{ $rowData->user?->userDetail?->regular_lic ?: asset('uploads/user_documents/default.jpg') }}" alt="Regular License Front">
                                    <div class="document-overlay">
                                        <i class="bx bx-search-alt"></i>
                                        <span>View Full Size</span>
                                    </div>
                        ...
                        <div class="document-card-inner">
                            <div class="document-header">
                                ...
                                @if(!empty($rowData->user?->userDetail?->regular_lic_back_status))
                                    <span class="status-badge status-{{ strtolower($rowData->user->userDetail->regular_lic_back_status) }}">
                                        {{ $rowData->user->userDetail->regular_lic_back_status }}
                                    </span>
                                @endif
                            </div>
                            <div class="document-image-wrapper">
                                {{--                                <a href="{{ $rowData->user?->userDetail?->regular_lic_back ?: asset('uploads/user_documents/default.jpg') }}" target="_blank" data-lightbox="gallery" class="document-link">--}}
                                <a href="javascript:void(0);"
                                   class="document-link openPreview"
                                   ...
                                   data-status="{{ $rowData->user?->userDetail?->regular_lic_back_status }}">
                                    <img class="document-image" src="{{ $rowData->user?->userDetail?->regular_lic_back ?: asset('uploads/user_documents/default.jpg') }}" alt="Regular License Back">
                                    <div class="document-overlay">
                                        <i class="bx bx-search-alt"></i>
                                        <span>View Full Size</span>
                                    </div>
            ...
            $('#btnSendSelected').on('click', function () {
                ...
                Swal.fire({
                    ...
                    if (selectedActions.send_document_verified) {
                        requests.push(
                            $.ajax({
                                url: "{{ route('admin.booking.document-verified-mail') }}", // Update this route
                                type: "POST",
                                dataType: "json",
                                data: {
                                    booking_id: bookingId,
                    ...
                    $.when.apply($, requests)
                        ...
                        .always(function () {
                            ...
                            $(document).on('click', '.documentVerifiedMail', function () {
                                ...
                                $.ajax({
                                    url: "{{ route('admin.booking.document-verified-mail') }}",
                                    method: "POST",
                                    dataType: "json",
                                    data: {
                                        booking_id: bookingId,
