<script>
    ...
    function actionError(xhr, message = "400 Bad Request") {
        if (xhr.status == 400) {
            ...
            sendError("Access to the requested resource is forbidden.")
        } else if (xhr.status === 500) {
            sendError("500 Internal Server Error")
        }
    }

    function uploadFilePond(file, filetype) {
        return new Promise(function (resolve, reject) {
            ...
            $.ajax({
                url: "{{route('admin.tinymce.file.upload')}}", // Laravel route for file uploads
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
