                ...
                <div class="specs-container">
                    <div class="specs-grid">
                        ...
                        <div class="specs-item">
                            ...
                            @php
                                $dbInteriorColor = trim($car->spec->interior_color ?? '');
                                $matchedInteriorHex = '';

                                if (!empty($dbInteriorColor) && strtoupper($dbInteriorColor) !== 'N/A') {
                                    if (preg_match('/^#[0-9A-Fa-f]{6}$/', $dbInteriorColor)) {
                                        $matchedInteriorHex = $dbInteriorColor;
                                    } else {
                                        $interiorColorMap = [
                                            ['name' => 'White', 'hex' => '#FFFFFF'],
                                            ['name' => 'Pearl', 'hex' => '#FDFDF0'],
                                        ...
                                        foreach ($interiorColorMap as $c) {
                                            if (strcasecmp($dbInteriorColor, $c['name']) === 0 || strcasecmp($dbInteriorColor, $c['hex']) === 0 || (strtolower($c['name']) !== 'blue' && strtolower($c['name']) !== 'l. blue' && strtolower($c['name']) !== 'white' && strtolower($c['name']) !== 'pearl' && str_contains(strtolower($dbInteriorColor), strtolower($c['name'])))) {
                                                $matchedInteriorHex = $c['hex'];
                                                break;
                                            }
                                        }
                                    }
                            ...
                            @endphp
                            <div class="color-display-wrapper">
                                @if(!empty($matchedInteriorHex))
                                    <div class="color-box" style="background-color: {{ $matchedInteriorHex }};"></div>
                                @else
                                    <span
                                        class="specs-value">{{ (!empty($dbInteriorColor) && strtoupper($dbInteriorColor) !== 'N/A') ? $dbInteriorColor : '-' }}</span>
                                @endif
                            </div>
                        </div>
                        <!-- Share Button -->
                    ...
                    <div id="shareMenu" class="share-menu" style="display: none; top: 100%; left: 0; margin-top: 8px;">
                        ...
                        <a id="shareThreads" href="#" target="_blank" rel="noopener">
                            Threads
                        </a>
